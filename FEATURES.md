...
- **Battle Master (Fighter, 2014)** — Combat Superiority die pool + cosmetic maneuver items; wired: **Ambush** (roll-configuration-dialog button on an initiative roll or a Dexterity (Stealth) check adds the die directly to that roll, refundable, provided the actor isn't incapacitated), **Bait and Switch** (bare-item USE/CHAT dialog posts a roll-superiority-die announcement; the roll's own chat message carries an apply-effects tray granting the recipient a temporary AC bonus equal to the roll, tracked for one round in the combat tracker), **Brace** (melee-only weapon usage-card button adds the die to that attack's damage roll on a hit, refundable), **Commander's Strike** (directs an ally's attack, refundable), **Commanding Presence** (roll-configuration-dialog button on a Charisma (Intimidation/Performance/Persuasion) check adds the die directly to that roll, refundable), **Disarming Attack** (bonus damage + Strength-save prompt, refundable), **Distracting Strike** (bonus damage + trackable "Distracted" marker, refundable), **Evasive Footwork** (rolls the die for an AC bonus applied via the effects tray, removed manually), **Feinting Attack** (USE/CHAT announcement with a roll-superiority-die button, granting Advantage on the caster's next attack against the target via a trackable marker), **Goading Attack** (weapon-card button adds the die to that attack's damage roll, refundable, and offers a "Goaded" apply-effects tray on the damage message that imposes Disadvantage on the target's attacks against anyone but the caster until the caster's next turn; the maneuver item can also be used standalone via a USE/CHAT dialog that posts a roll-superiority-die announcement), **Grappling Strike** (melee-only attack-roll-message button triggers the attacker's own Strength (Athletics) check with the die pre-added, refundable), **Lunging Attack** (weapon usage-card button adds the die to that melee attack's damage roll, refundable), **Maneuvering Attack** (attack-roll-message button adds the die to that attack's damage roll, refundable), **Menacing Attack** (attack-roll-message button adds the die to that attack's damage roll, refundable; the resulting damage message gets a Wisdom-save prompt at the maneuver's DC, rolled by targeted tokens or, if none, controlled tokens), **Precision Attack** (usage-card button adds the die directly to that weapon's next attack roll, refundable), **Pushing Attack** (attack-roll-message button adds the die to that attack's damage roll, refundable; the resulting damage message gets a Strength-save prompt at the maneuver's DC, rolled by targeted tokens or, if none, controlled tokens), **Quick Toss** (ranged-attack-with-a-thrown-weapon attack-roll-message button adds the die to that attack's damage roll, refundable), **Rally** (bare-item USE/CHAT dialog posts a roll-superiority-die announcement with a refundable roll↔refund toggle, mirroring Commander's Strike), **Riposte** (melee-only weapon usage-card button adds the die to that attack's damage roll, refundable), **Sweeping Attack** (melee-only attack-roll-message button immediately rolls and posts a separate typed damage message equal to the die, refundable), **Tactical Assessment** (roll-configuration-dialog button on an Intelligence (Investigation/History) or Wisdom (Insight) check adds the die directly to that roll, refundable), **Trip Attack** (attack-roll-message button adds the die to that attack's damage roll, refundable; the resulting damage message gets a Strength-save prompt at the maneuver's DC, rolled by targeted tokens or, if none, controlled tokens, automatically knocking any that fail prone).
