...
- **Battle Master (Fighter, 2014)** — Combat Superiority die pool + cosmetic maneuver items; wired: **Commander's Strike** (directs an ally's attack, refundable), **Disarming Attack** (bonus damage + Strength-save prompt, refundable), **Distracting Strike** (bonus damage + trackable "Distracted" marker, refundable), **Evasive Footwork** (rolls the die for an AC bonus applied via the effects tray, removed manually), **Feinting Attack** (USE/CHAT announcement with a roll-superiority-die button, granting Advantage on the caster's next attack against the target via a trackable marker), **Goading Attack** (weapon-card button adds the die to that attack's damage roll, refundable, and offers a "Goaded" apply-effects tray on the damage message that imposes Disadvantage on the target's attacks against anyone but the caster until the caster's next turn; the maneuver item can also be used standalone via a USE/CHAT dialog that posts a roll-superiority-die announcement), **Lunging Attack** (weapon usage-card button adds the die to that melee attack's damage roll, refundable), **Maneuvering Attack** (attack-roll-message button adds the die to that attack's damage roll, refundable).
