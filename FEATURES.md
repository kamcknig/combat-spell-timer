...
- **Battle Master (Fighter, 2014)** — Combat Superiority die pool + cosmetic maneuver items; wired maneuvers (refundable before their die is consumed): **Commander's Strike** (directs an ally's attack), **Disarming Attack** (bonus damage + Strength-save prompt), **Distracting Strike** (bonus damage + trackable "Distracted" marker on the target).
